...
var ModelNameNormalizer = /** @class */ (function () {
    ...
    ModelNameNormalizer.prototype.normalizeField = function (field, parentType, parentModel) {
        ...
        if (!parentType.isEnum && !field.isId) {
            if (field.name !== camelcase_1.default(field.name)) {
                // if we can't find another field with the camelcased name, we're save
                // and don't have a collision
                if (!parentType.fields.find(function (f) { return f.name === camelcase_1.default(field.name); })) {
                    field.databaseName = field.name;
                    field.name = camelcase_1.default(field.name);
                }
            }
        }
